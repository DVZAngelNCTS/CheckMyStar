...
namespace CheckMyStar.Dal.Abstractions
{
    public interface IAddressDal
    {
        ...
        Task<AddressResult> GetAddress(int identifier, CancellationToken ct);
        ...
        Task<AddressesResult> GetAddresses(string? number, string? addressLine, string? city, string? zipCode, string? region, int? countryIdentifier, CancellationToken ct);
    }
}
