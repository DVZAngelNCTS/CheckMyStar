...
export class FrontDossiersPageComponent implements OnInit {
  ...
  loadingReset = false;
  folders: FolderModel[] = [];
  currentInspector: UserModel | null = null;
  @ViewChild(FrontDossierFormComponent) folderForm!: FrontDossierFormComponent;
  ...
  loadFolders() {
		this.folderBll.getfoldersByInspector$(this.currentInspector?.identifier!).subscribe({
  ...
  onFilter(filter: FolderFilter) {
    ...
    this.folderBll.getfoldersByInspector$(this.currentInspector?.identifier!, filter.accommodationName, filter.ownerLastName, filter.inspectorLastName, filter.folderStatus ?? undefined).subscribe({
			next: folders => {
				if (filter.reset)
  ...
  onEditConfirmed() {
    ...
    updatedFolder.inspector = this.currentInspector ?? undefined;
